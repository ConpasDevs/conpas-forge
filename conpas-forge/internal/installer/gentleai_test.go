...
func TestGentleAISkillCount(t *testing.T) {
	tests := []struct {
		...
		{name: "matches embedded skill list", want: 21},
	}
...
func TestCountGentleAISkillsDeployedMatchesSourceList(t *testing.T) {
	...
	if got := fmt.Sprintf("%d skills", CountGentleAISkillsDeployed(paths)); got != "21 skills" {
		t.Fatalf("formatted deployed skill count = %q, want %q", got, "21 skills")
	}
}
